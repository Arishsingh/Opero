...
import { useAnimate, stagger as motionStagger, type Transition } from "framer-motion";
...
type Props = {
  ...
  animate?: boolean;
  /** When false, messages stay hidden (but laid out) until it flips to true, e.g. on scroll into view. */
  play?: boolean;
  staggerDelay?: number;
...
// Matches HIDDEN so items don't flash visible before the first animation frame.
const HIDDEN_STYLE: React.CSSProperties = { opacity: 0, transform: "translateY(10px) scale(0.85)" };

const DEFAULT_MESSAGES: Message[] = [
  { text: "Hey! Are you coming to the Framer workshop?", sender: "them", timestamp: "10:14 AM" },
  { text: "Yeah, definitely! Just finishing up some code.", sender: "me", timestamp: "10:15 AM" },
  { text: "Awesome, see you there! 🚀", sender: "them", timestamp: "10:16 AM" },
];
...
const DEFAULT_TRANSITION: Transition = { type: "spring", stiffness: 450, damping: 28, mass: 1 };

function TypingBubble({ bubbleColor, isMe }: { bubbleColor: string; isMe: boolean }) {
  return (
    <div
      style={{
        display: "inline-flex",
...
export default function IOSMessageList({
  ...
  const initialStyle = doAnimate ? HIDDEN_STYLE : undefined;
  ...
  const runAppear = useCallback(() => {
    if (!scope.current?.querySelectorAll(".msg-item").length) return;
    animate(".msg-item", SHOWN, { ...baseTransition, delay: motionStagger(staggerSec) });
  ...
  }, [doAnimate, play, runAppear, resetToHidden, runDots, animate, messages.length, showTyping]);
  ...
  return (
    ...
    >
      ...
      {showTyping && (
        <div style={{ display: "flex", justifyContent: isTypingFromMe ? "flex-end" : "flex-start", width: "100%", marginTop: 4 }}>
          <div className="msg-item" style={{ display: "inline-flex", willChange: "transform, opacity", ...initialStyle }}>
            <TypingBubble bubbleColor={isTypingFromMe ? sentBubbleColor : receivedBubbleColor} isMe={isTypingFromMe} />
          </div>
        </div>
      )}
    </div>
